...
func (s *Session) Write(key string, data []byte, offset, total_size uint64, onLookup onLookupCallback, onResult onResultCallback) {
    var err error
    ekey, err := NewKey(key)
    ...
    s.write(ekey, data, offset, total_size, onLookup, onResult)
}

func (s *Session) write(key *Key, data []byte, offset, total_size uint64, onLookup onLookupCallback, onResult onResultCallback) {
    ...
    if onLookup == nil {
        onLookup = func(lookup *lookupResult) {

        }
    }

    Pool.Store(onWriteContext, onLookup)
    Pool.Store(onWriteFinishContext, onWriteFinish)
    Pool.Store(chunk_context, data)

    C.session_write_data(s.session,
    C.context_t(onWriteContext), C.context_t(onWriteFinishContext),
    key.key, C.uint64_t(offset), (*C.char)(unsafe.Pointer(&data[0])), C.uint64_t(len(data)))
}
...
func (s *Session) cache(key *Key, data []byte, timeout uint64, onLookup onLookupCallback, onResult onResultCallback) {
    onWriteContext := NextContext()
    onWriteFinishContext := NextContext()
    chunk_context := NextContext()
    ...
    C.context_t(onWriteContext), C.context_t(onWriteFinishContext),
    key.key, (*C.char)(unsafe.Pointer(&data[0])),
    C.long(timeout), C.uint64_t(len(data)),
    )
}

func (s *Session) Cache(key string, data []byte, timeout uint64, onLookup onLookupCallback, onResult onResultCallback) {
    ekey, err := NewKey(key)
    defer func(){
        if err != nil && onResult != nil {
            onResult(err)
    ...
    if len(data) == 0 {
        err = errors.New("Invalid zero-length write request")
        return
    }
    s.cache(ekey, data, timeout, onLookup, onResult)
}
...
func (s *Session) WriteCache(key string, input io.Reader, timeout uint64) <-chan Lookuper {
    ...
    s.cache(ekey, chunk, timeout, onWriteResult, onWriteFinish)

    return responseCh
}

type onLookupCallback func(lookup *lookupResult)
type onResultCallback func(err error)
...
func (s *Session) WriteKey(key *Key, input io.Reader, offset, total_size uint64) <-chan Lookuper {
    ...
    s.write(key, chunk, offset, total_size, onWriteResult, onWriteFinish)
    return responseCh
}
