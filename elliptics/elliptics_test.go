...
Setup env variables. Example: export %s="localhost:1025:2" && export %s="1,2,3"`,
    ...
    // Read after Remove
    for res := range session.ReadData(testKey, 0, 0){
        t.Logf("ReadNotFoundData %s: %v", testKey, res.Error())
    }
    ...
    for res := range session.WriteCache(testKey+"_double", testBlobReader, 5) {
        if err := res.Error(); err != nil {
            t.Fatalf("session.WriteCache error: %s", err)
        }
    }*/
    bulkReadKeyOne := fmt.Sprintf("bulk_one_%d", time.Now().Unix())
    bulkReadKeyTwo := fmt.Sprintf("bulk_two_%d", time.Now().Unix())

    testBlobReader = strings.NewReader(TEST_BLOB)
    for res := range session.WriteData(bulkReadKeyOne, testBlobReader, 0, 0) {
        if err := res.Error(); err != nil {
            t.Fatalf("lookup result error: %s", err)
        }
    }

    testBlobReader = strings.NewReader(TEST_BLOB+"____2")
    for res := range session.WriteData(bulkReadKeyTwo, testBlobReader, 0, 0) {
        if err := res.Error(); err != nil {
            t.Fatalf("lookup result error: %s", err)
        }
    }
    t.Logf("Start bulking read")

    keys_l := []string{bulkReadKeyOne, bulkReadKeyTwo}
    results := session.BulkRead(keys_l)
    time.Sleep(time.Second)
    retrived_cnt := 0

    for res := range results {
        t.Logf("BulkRead-> '%s' : '%s'/'%s'", key, res.Data())
        if res.Key() == bulkReadKeyOne || res.Key() == bulkReadKeyTwo {
            retrived_cnt += 1
        }
    }

    if retrived_cnt != 2 {
        t.Errorf("Bulk read returned lesser data, then expected: %d", retrived_cnt)
    }

    for res := range session.BulkRemove(keys_l) {
        if res.Error() != nil {
            t.Error(err)
        }
    }
    ...
    bulkKeyOne := fmt.Sprintf("test_bulked_1")
    bulkKeyTwo := fmt.Sprintf("test_bulked_2")

    firstAttr := new(DnetIOAttr)
    secondAttr := new(DnetIOAttr)

    firstAttr.ID = []byte(bulkKeyOne)
    secondAttr.ID = []byte(bulkKeyTwo)

    attrs := []*DnetIOAttr{firstAttr, secondAttr}
    dataset := [][]byte{[]byte(TEST_BLOB+"__4"), []byte(TEST_BLOB+"__3")}

    for res := range session.BulkWrite(attrs, dataset){
        if res.Error() != nil {
            t.Error(err)
            continue
        }
    ...
    retrived_cnt = 0
    bkeys := []string{bulkKeyOne, bulkKeyTwo}

    for res := range session.ReadData(bulkKeyOne, 0, 0){
        if res.Error() != nil {
            t.Error(res.Error())
        } else {
            t.Logf("Received: '%s'/'%s'", bulkKeyOne, res.Data())
        }
    }

    for res := range session.ReadData(bulkKeyTwo, 0, 0){
        if res.Error() != nil {
            t.Error(res.Error())
        } else {
            t.Logf("Received: '%s'/'%s'", bulkKeyTwo, res.Data())
        }
    }
    time.Sleep(time.Second)
    results = session.BulkRead(bkeys)
    for res := range results {
        if res.Error() != nil {
            t.Error(res.Error())
            continue
        }
        if res.Key() == bulkKeyTwo || res.Key() == bulkKeyOne {
            retrived_cnt += 1
        }
    }

    if retrived_cnt < 2 {
        t.Errorf("AfterWriteBulk read returned lesser data, then expected: %d", retrived_cnt)
    }



    for res := range session.BulkRemove(bkeys){
        res.Error()
    }
    ...
    for res := range results {
        if res.Error() != nil {
            t.Logf("AfterRemove %s", res.Error().Error())
            continue
        }
    }
}
